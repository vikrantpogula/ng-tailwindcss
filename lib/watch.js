...
module.exports = () => {
  ...
  if (fs.existsSync(ngTwFile)) {
    ...
    const tailwind = chokidar.watch([config.configJS, config.sourceCSS, config.watchPaths])
    ...
    printWatchedFiles(config.configJS, config.sourceCSS, config.watchPaths, ngTwFile)
  } else {
